...
def find_all_combinations(i, adapters, seen):
    if i == len(adapters)-1:
        return 1
    if i in seen:
        return seen[i]
    ...
    for j in range(i + 1, len(adapters)):
        if adapters[j] - adapters[i] <= 3:
            ans += find_all_combinations(j, adapters, seen)
    seen[i] = ans
    return ans
...
if __name__ == "__main__":
    with open("Day_10/input.txt", "r") as in_file:
        ...
        joltages = sorted([0, *joltages, max(joltages)+3])
        print(check_adapters(joltages))
        print(find_all_combinations(0, joltages, {}))
