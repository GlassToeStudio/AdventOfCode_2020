...
def format_data(data):
    map = {}
    for line in data.read().replace('bags', 'bag').replace('.', '').split('\n'):
        k, v = line.split(' contain ')
        if 'no other' in v:
            continue
        if k in map:
            map[k].append({x[2:]: int(x[0]) for x in v.split(', ')})
        else:
            map[k] = {x[2:]: int(x[0]) for x in v.split(', ')}
    return map
...
if __name__ == "__main__":
    with open("Day_07/input.txt", "r") as in_file:
        ...
        found = {}
        name = 'shiny gold bag'
        found = bags_with_gold_bags(bags, name, found)
